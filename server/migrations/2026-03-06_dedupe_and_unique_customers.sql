...
-- 2) Delete duplicates keeping the lowest id (UNCOMMENT TO RUN after review)
...
-- 3) Create unique index after duplicates removed
CREATE UNIQUE INDEX IF NOT EXISTS customers_external_id_unique
  ON customers(external_id);

-- 4) Ensure latest schema columns exist (idempotent)
